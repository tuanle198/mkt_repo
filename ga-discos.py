...
df['type'] = df['Landing Page'].apply(lambda x: x.split('surface_type')[1].split('=')[1] if 'surface_type' in x else 'Others')

# Extract keyword
df['keyword'] = df['Landing Page'].apply(lambda x: x.split('surface_detail=')[1].split('&')[0] if 'surface_detail' and 'surface_type=search' in x else 'Others')
...
keyword_group = df[df['keyword'] != 'Others'].groupby('keyword').agg({'Users':'sum','Pages/Session':'mean', 'Avg. Session Duration':'mean'}).reset_index()
keyword_group['Target Priority'] = 'None'
keyword_group.loc[keyword_group['keyword'].apply(lambda x: x in desc),'Target Priority'] = 'Include in desc.'
keyword_group.loc[keyword_group['keyword'].apply(lambda x: x in main),'Target Priority'] = 'Main'
top_keyword_group = keyword_group.sort_values('Users', ascending = False).head(10)
...
with fig_col1:
    ...
    fig = px.bar(top_keyword_group, x="Users", y="keyword", color = 'Target Priority')
    st.write(fig)
   
with fig_col2:
    st.markdown("### Average page per session (all time, top 10 keywords)")
    fig = px.bar(top_keyword_group, y="Pages/Session", x="keyword", color = 'Target Priority')
    st.write(fig)

fig_col3, fig_col4 = st.columns(2)

with fig_col3:
    st.markdown("### Average session duration in seconds (all time, top 10 keywords)")
    fig2 = px.bar(data_frame= top_keyword_group, x="keyword", y = "Avg. Session Duration", color = 'Target Priority')
    st.write(fig2)
